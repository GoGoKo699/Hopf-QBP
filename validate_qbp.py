...
ANALYTIC_MODULES = (
    ...
    "qbp_validation.tests.test_ancilla_depth_compiler",
    "qbp_validation.tests.test_supporting_analysis",
)

SMOKE_TESTS = (
    "qbp_validation.tests.test_circuit_conventions",
    "qbp_validation.tests.test_operator_contracts.OperatorContractTests.test_real_native_depth_and_frame_share_only_state_column",
    "qbp_validation.tests.test_operator_contracts.OperatorContractTests.test_checkpoint_interface_preserves_means_not_complete_distributions",
    "qbp_validation.tests.test_real_frame.RealFrameTests.test_regular_frames_n1_to_n4",
    "qbp_validation.tests.test_complex_frame.ComplexFrameTests.test_addressed_rc_compiler_is_complete_frame_up_to_common_phase",
    ...
    "qbp_validation.tests.test_checkpoints.CheckpointTests.test_integrated_complex_depth2_checkpoint",
)
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument(
        ...
        help=(
            ...
            "compiler, ancilla-depth, and supporting-analysis checks."
        ),
    )
    mode.add_argument(
        "--smoke",
        action="store_true",
        help="Run the analytic checks plus representative Qibo circuit contracts.",
    )
    parser.add_argument(
        "--quiet", action="store_true", help="Use compact unittest output."
    )
...
def main() -> int:
    ...
    if args.analytic:
        ...
        print(
            f"Qibo: {qibo_version} "
            "(NumPy backend selected by qbp_validation.circuits)",
            flush=True,
        )
        if args.smoke:
            suite = unittest.TestSuite()
