...
- `theta_mag[j - 1]` stores \(\theta_j\), for \(1\le j\le N-1\). Thus
  ...
  \(\boldsymbol\theta_{\mathrm{mag}}=(\theta_1,\ldots,\theta_{N-1})\).
- `theta_ph[ell]` stores \(\theta_{N+\ell}\), for \(0\le\ell<N\). Thus
  `theta_ph` represents
  \(\boldsymbol\theta_{\mathrm{ph}}=(\theta_N,\ldots,\theta_{2N-1})\).
...
- `HopfReal`, \(U_{\mathrm{chk}}\), and \(W_{\mathbb R}\) prepare the same state
  from \(\lvert0\rangle\), but they are distinct full-unitary completions.
- The complex global-magnitude circuit uses the literal asymmetric sequence

  \[
  U_{\mathrm{chk}}
  \longrightarrow D_{\mathrm{ph}}
  \longrightarrow \operatorname{ctrl}(O)
  \longrightarrow D_{\mathrm{ph}}^\dagger
  \longrightarrow W_{\mathbb R}^\dagger.
  \]
...
| [`test_real_frame.py`](qbp_validation/tests/test_real_frame.py) | \(W_{\mathbb R}\lvert0\rangle=\lvert\psi\rangle\) and \(W_{\mathbb R}\lvert\lambda(j)\rangle=\lvert e_j\rangle\), including singular completions. |
...
\(3\times10^{-12}\). Residuals at floating-point roundoff scale should be read as
