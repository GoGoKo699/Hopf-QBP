...
class RealFrameTests(unittest.TestCase):
    def _check_case(self, theta_mag: np.ndarray) -> None:
        data = real_tree_data(theta_mag)
        N = 1 << data.n
        qibo_unitary = np.asarray(frame_circuit(theta_mag).unitary())
        reference = real_frame_matrix(theta_mag)
        np.testing.assert_allclose(qibo_unitary, reference, atol=2e-12, rtol=0.0)
        np.testing.assert_allclose(
            qibo_unitary.conj().T @ qibo_unitary,
            np.eye(N),
            atol=2e-12,
            rtol=0.0,
        )
        ...
        for node in range(1, N):
            ...
            np.testing.assert_allclose(
                qibo_unitary[:, marker], complement, atol=2e-12, rtol=0.0
            )
            np.testing.assert_allclose(
                data.derivatives[node - 1],
                data.sqrt_metric[node - 1] * complement,
                atol=2e-12,
                rtol=0.0,
            )

        depth_state = statevector(depth_preparation_circuit(theta_mag))
        np.testing.assert_allclose(depth_state, data.state, atol=2e-12, rtol=0.0)

    def test_regular_frames_n1_to_n4(self) -> None:
        for n in range(1, 5):
            with self.subTest(n=n):
                self._check_case(regular_theta_mag(n))

    def test_singular_frame_completion_remains_unitary(self) -> None:
        for n in range(1, 5):
            with self.subTest(n=n):
                self._check_case(singular_theta_mag(n))
