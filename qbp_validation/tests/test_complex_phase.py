...
from qbp_validation.circuits import (
    ...
    complex_phase_native_forward_circuit,
    probabilities,
)
...
class ComplexPhaseTests(unittest.TestCase):
        ...
        for n in range(1, 5):
            ...
            for obs_index, observable in enumerate(observables(n)):
                with self.subTest(n=n, observable=obs_index):
                    ...
                    exact = complex_phase_gradient(mag, phase, observable)
    ...
    def test_native_forward_has_same_complete_distribution(self) -> None:
        ...
        separated = probabilities(complex_phase_measurement_circuit(mag, phase, observable))
        native = probabilities(complex_phase_native_forward_circuit(mag, phase, observable))
        np.testing.assert_allclose(native, separated, atol=3e-12, rtol=0.0)
            ...
            for ancilla in (0, 1):
                for leaf in range(N):
                    self.assertAlmostEqual(
                        float(np.linalg.norm(phase_record(ancilla, leaf, N))), 2.0
                    )
    ...
    def test_zero_amplitude_leaf_phase_derivatives_vanish(self) -> None:
        ...
        decoded = decode_phase_gradient(
            probabilities(complex_phase_measurement_circuit(mag, phase, observable))
        )
        exact = complex_phase_gradient(mag, phase, observable)
        np.testing.assert_allclose(decoded, exact, atol=3e-12, rtol=0.0)
